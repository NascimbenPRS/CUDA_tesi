...
// sum integers dataArr[j], where j belongs to a constraint (array of indexes) on GPU.
// Propagate only constraints of size == constLength
__global__ void constraintSumGPU_fixedLength_Arrays(int *dataArr, int arraySize, int *sumValues, int numCycles, int numConstraints, int **constraints, int *constraintSizes, int constLength) {
	int index = threadIdx.x + (blockIdx.x * blockDim.x);
	if (index < sizesAmount) {
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	for (int i= 0; i < numConstraints; i++){
        constraintsOfSizes[constraintSizes[i]][tempCounter[constraintSizes[i]]]= i;
	...
	for (int constLength = 4; constLength <= maxConstraintSize; constLength++) {
            // DA FIXARE I PARAMETRI
		constraintSumGPU_fixedLength_arrays << <numBlocksArray[constLength], blockSizeArray[constLength], 0, streams[constLength] >> > (dataArray, arraySize, resultsGPU, numCyclesGPU, numConstraints, constraints, constraintSizes, constLength);
	}
	cudaDeviceSynchronize();
	endClock = clock();
	...
	for (int i = 0; i < maxConstraintSize + 1; i++){
        genericFree(constraintsOfSizes[i]);
	}
	genericFree(constraintsOfSizes);
