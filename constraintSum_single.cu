...
int main(int argc, char *argv[])
{
	...
	*/
	...
	//		Propagate constraints of each size in [0, maxConstraintSize]
	for (int constLength = 0; constLength <= maxConstraintSize; constLength++) {
		startClock = clock();
		constraintSumGPU_fixedLength << <numBlocks, blockSize >> > (dataArray, arraySize, resultsGPU, numCyclesGPU, numConstraints, constraints, constraintSizes, constLength);
		cudaDeviceSynchronize();
		endClock = clock();
