...
int main(int argc, char *argv[])
{
	...
	*/
	...
	for (int constLength = 4; constLength <= maxConstraintSize; constLength++) {	
		constraintSumGPU_fixedLength << <numBlocks, blockSize, 0, streams[constLength] >> > (dataArray, arraySize, resultsGPU, numCyclesGPU, numConstraints, constraints, constraintSizes, constLength);
	}
	cudaDeviceSynchronize();
	endClock = clock();
