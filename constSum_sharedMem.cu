...
__global__ void constraintSumGPU_sharedMem(int *dataArr, int arraySize, int *sumValues, int numCycles, int **constraints, int constraintIndex, int constraintSize) {
	__shared__ int currentConstraintData[128];
	int index = threadIdx.x + (blockIdx.x * blockDim.x);
	if (index < constraintSize) {
		// current thread can store its data in shared memory
		currentConstraintData[index] = dataArr[constraints[constraintIndex][index]];

		int tempSum = 0;
		for (int k = 0; k < numCycles; k++) {
			tempSum = 0;
			tempSum += currentConstraintData[index];
		}
		sumValues[constraintIndex] += tempSum;
	}
	else {
		// invalid constraint index
	}
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	printf("\nA kernel for each constraint, using shared memory.\n");
	...
	*/
	...
	genericMalloc((void**)&streams, numConstraints * sizeof(cudaStream_t));
	for (int i = 0; i < numConstraints; i++) {
	...
	//		Launch a kernel for each constraint, loading data in shared memory.
	startClock = clock();
	// launch all kernels
	for (int constrIndex = 0; constrIndex < numConstraints; constrIndex++) {
		constraintSumGPU_sharedMem << <1, 128, 128*sizeof(int), streams[constrIndex] >> > (dataArray, arraySize, resultsGPU, numCyclesGPU, constraints, constrIndex, constraintSizes[constrIndex]);
	}
	cudaDeviceSynchronize();
	endClock = clock();
